...
function applyBrandingAndUi(source){
  let html=source;
  const logo='https://raw.githubusercontent.com/comercialrazuck/cotacao-yepii-25-09-26/feature/cotacao-email-reports/assets/yepii-logo.png';
  ...
  const forceClientUi=`<script>(function(){function ensureBuyerUi(){if(!location.pathname.startsWith('/c/'))return;document.body.classList.add('client-view');var word=document.querySelector('.wordmark');if(word){var old=word.querySelector('.logo');if(old){var img=document.createElement('img');img.className='official-yepii-logo';img.alt='Yepii';img.src='${logo}';old.replaceWith(img)}else if(!word.querySelector('.official-yepii-logo')){var img2=document.createElement('img');img2.className='official-yepii-logo';img2.alt='Yepii';img2.src='${logo}';word.prepend(img2)}}var terms=document.querySelector('.terms');if(terms&&!document.getElementById('clientPrintQuoteBtn')){var row=document.createElement('div');row.className='client-print-row';var btn=document.createElement('button');btn.id='clientPrintQuoteBtn';btn.type='button';btn.className='btn muted client-print-btn';btn.innerHTML='<i class="fa-solid fa-print"></i> Imprimir cotação para aprovação';btn.addEventListener('click',function(){if(typeof window.printQuote==='function'){window.printQuote()}else{window.print()}});row.appendChild(btn);terms.parentNode.insertBefore(row,terms)}}document.addEventListener('DOMContentLoaded',ensureBuyerUi);setTimeout(ensureBuyerUi,300);setTimeout(ensureBuyerUi,1200)})();</script>`;
